...
text(W - 48, 58, "Rev D", 13, "end", fill=NOTE)
...
text(ax + aw / 2, ay + ah + 22, "temperature + humidity", 12, "middle", fill=NOTE)
text(ax + aw / 2, ay + ah + 38, "3-pin module, single-wire", 12, "middle", fill=NOTE)

am_pins = {"VCC": ay + 54, "DAT": ay + 80, "GND": ay + 104}
for nm, y in am_pins.items():
    wire([(ax, y), (ax - 24, y)])
    text(ax + 12, y + 4, nm, 12, "start", fill=NOTE)

wire([(ax - 24, am_pins["VCC"]), (ax - 24, Y_3V3)], RAIL_POS)
junction(ax - 24, Y_3V3, RAIL_POS)

wire([(ax - 58, am_pins["GND"]), (ax - 24, am_pins["GND"])], RAIL_GND)
wire([(ax - 58, am_pins["GND"]), (ax - 58, Y_GBUS)], RAIL_GND)
junction(ax - 58, Y_GBUS, RAIL_GND)

X_DAT = 430
wire([(ax - 24, am_pins["DAT"]), (X_DAT, am_pins["DAT"]), (X_DAT, pin_y["D2"]),
      (X_EDGE, pin_y["D2"])])

text(392, 156, "Data-line pull-up is the STM32 internal pull-up,", 12.5, "start",
     fill=RAIL_POS, weight="600")
text(392, 172, "enabled in firmware. Many modules also carry one.", 12.5,
     "start", fill=NOTE)
text(392, 188, "If reads fail checksum, fit a 4.7 k\u03a9 to 3V3.", 12.5, "start", fill=NOTE)

text(452, 472, "Three wires and one sensor.", 13, "start", weight="600")
text(452, 492, "The AM2320 reports both chamber temperature and headspace", 12.5, "start", fill=NOTE)
text(452, 508, "humidity over one data line, so no analog channel, no divider", 12.5, "start", fill=NOTE)
text(452, 524, "and no I\u00b2C bus are needed. Mount it inside the headspace.", 12.5, "start", fill=NOTE)
...
notes = [
    ...
    ("One component total: the AM2320, on a DHT-compatible single-wire bus. A 4-pin I2C module works too", False),
    ("with SENSOR_SINGLE_WIRE set to 0 in the sketch; a 3-pin module cannot do I2C at all.", False),
]
