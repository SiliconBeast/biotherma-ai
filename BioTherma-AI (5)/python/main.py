...
# Sent with every process update so the dashboard can populate its feed
# selector without having to ask for it.
FEEDSTOCK_LIST = [
...
class BioTherma:
    ...
    def on_telemetry(self, air_x100, rh_x100, probe_x100, err_count):
        ...
        if missing:
            ...
            ui.send_message("sample", self.last)
            return
    ...
    def push_process(self, probe_c):
        ...
        snap["feedstocks"] = FEEDSTOCK_LIST
        ui.send_message("process", snap)
        return snap
...
def on_history(client, data):
    rows = app.store.recent(720)
    ui.send_message("history", [
        {"ts": r[0], "air_c": r[1], "rh": r[2], "probe_c": r[3],
         "index": r[4], "state": r[5]} for r in rows
    ], client)
    ui.send_message("feedstocks", FEEDSTOCK_LIST, client)
    app.push_process(_probe())


def on_add_feed(client, data):
    data = data or {}
    try:
        feed = app.add_feed(data["kind"], float(data["kg"]))
    except (KeyError, ValueError, TypeError) as e:
        ui.send_message("error", {"where": "add_feed", "message": str(e)}, client)
        return
    ...
    if app.transition(app.classify(_probe(), app.last.get("z_peak"))):
        app.push_leds()
    ui.send_message("fed", {"feed": feed.as_dict(), "cn": snap["cn"]})


def on_harvest(client, data):
    app.harvest()
    app.state = "INIT"
    app.push_leds()
    app.push_process(_probe())
...
def on_set_array(client, data):
    app.array_on = bool((data or {}).get("on", True))
    app.push_leds()
    ui.send_message("array", {"on": app.array_on})


def on_lamp_test(client, data):
...
ui.on_message("history", on_history)
ui.on_message("add_feed", on_add_feed)
ui.on_message("harvest", on_harvest)
ui.on_message("set_price", on_set_price)
ui.on_message("set_array", on_set_array)
ui.on_message("lamp_test", on_lamp_test)
...
def heartbeat():
    ...
    while True:
        ...
        if last_ts and time.time() - last_ts > 15 and app.state != "INIT":
            app.state = "INIT"
            app.push_leds()
            ui.send_message("sample", {"state": "INIT", "fault": "mcu_silent"})
