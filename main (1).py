...
FEATURES = [f for f in
            os.environ.get("BIOTHERMA_FEATURES", ",".join(ALL_FEATURES)).split(",")
            if f.strip() in ALL_FEATURES] or list(ALL_FEATURES)
...
LED_GREEN, LED_YELLOW, LED_RED = 0x01, 0x02, 0x04
...
class BioTherma:
    ...
    def leds_for(self, state):
        return {
            "PRODUCING": LED_GREEN,
            "HARVEST":   LED_YELLOW,
            "BALANCE":   LED_YELLOW | LED_GREEN,
            "WATCH":     LED_YELLOW,
            "STALL":     LED_RED,
            "BAND":      LED_RED | LED_YELLOW,
            "EMPTY":     0,
            "INIT":      0,
        }.get(state, 0)
    ...
    def push_leds(self):
        try:
            Bridge.call("set_status", int(self.leds_for(self.state)),
                        1 if self.array_on else 0)
        except Exception as e:
            print(f"[biotherma] bridge set_status failed: {e}")

    # -- classification --------------------------------------------------
    def classify(self, probe_c, z_peak):
        # Hard faults first. A thermal excursion outranks anything the model says.
        if probe_c is not None and not (MESO_LOW_C <= probe_c <= MESO_HIGH_C):
            return "BAND"
    ...
    def on_telemetry(self, air_x100, rh_x100, probe_x100, err_count):
        ...
        if probe is None and "probe_c" not in missing:
            missing.append("probe_c")
        if missing:
        ...
        # Integrate the batch against the measured slurry temperature.
        with self.lock:
            self.batch.update(probe, now=ts)

        candidate = self.classify(probe, z_peak)
...
def on_lamp_test(_=None):
    for mask in (LED_GREEN, LED_YELLOW, LED_RED, 0x07, 0x00):
        Bridge.call("set_status", mask, 1)
        time.sleep(0.35)
    app.push_leds()
